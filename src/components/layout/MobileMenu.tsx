    ...
    <div className="lg:hidden">
      ...
      {open && (
        <div
          aria-hidden="true"
          onClick={closeMenu}
          className="fixed inset-x-0 top-[var(--header-height)] bottom-0 z-[990] bg-black/20 backdrop-blur-sm transition-opacity duration-300 animate-in fade-in-0"
        />
      )}
      ...
      {open && (
        <>
          ...
          <button
            ...
            aria-expanded={open}
            className="fixed top-[calc(var(--header-height)+1rem)] right-6 z-[1000] flex h-11 w-11 items-center justify-center rounded-full border border-border/80 bg-white text-primary shadow-[0_4px_16px_rgba(0,0,0,0.1)] transition-all duration-250 ease-out hover:border-brand/40 hover:bg-section hover:scale-105 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 animate-in fade-in-50 zoom-in-90"
          >
          ...
          <div className="fixed inset-x-0 top-[var(--header-height)] bottom-0 z-[999] flex flex-col overflow-y-auto border-b border-border/80 bg-white/95 px-6 py-6 shadow-[0_20px_40px_rgba(0,0,0,0.08)] backdrop-blur-xl transition-all duration-300 ease-out animate-in slide-in-from-top-2 pb-[max(2rem,env(safe-area-inset-bottom))]">
